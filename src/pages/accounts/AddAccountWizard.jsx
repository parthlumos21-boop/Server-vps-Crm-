...
const normalizeCompanyBucket = (value) => {
  const normalized = String(value || '').trim().toLowerCase()
  if (normalized === 'swati' || normalized === 'lumos') return normalized
  return ''
}

const normalizeOwnerCode = (value) => String(value || '').trim()

const getOwnerName = (user = {}) => String(user.name || user.username || user.email || '').trim()

const buildOwnerOptionsForCompany = (users = [], company) => {
  const companyBucket = normalizeCompanyBucket(company)
  if (!companyBucket) return []

  const uniqueUsers = new Map()

  users
    .filter((user) => normalizeCompanyBucket(user.company) === companyBucket)
    .filter((user) => getOwnerName(user) && normalizeOwnerCode(user.ownerCode))
    .forEach((user) => {
      const id = String(user.id || user.legacyId || user.email || getOwnerName(user)).trim()
      if (!id || uniqueUsers.has(id)) return

      uniqueUsers.set(id, {
        value: id,
        label: getOwnerName(user),
        user,
      })
    })
...
const AddAccountWizard = () => {
  ...
  React.useEffect(() => {
    const loadOwners = async () => {
      try {
        ...
        setOwnerUsers(Array.isArray(users) ? users : [])
      } catch (err) {
  ...
  const activeOwners = useMemo(
    () => buildOwnerOptionsForCompany(ownerUsers, formData.accountCategory),
    [formData.accountCategory, ownerUsers]
  )

  const selectedOwnerOption = useMemo(
    () => activeOwners.find((owner) => owner.value === formData.accountOwner) || null,
    [activeOwners, formData.accountOwner]
  )
  ...
  const handleChange = (name, value) => {
    setFormData((prev) => ({
      ...prev,
      [name]: value,
      ...(name === 'accountCategory' ? { accountOwner: '' } : {}),
    }))
  ...
  const handleSubmit = async (event) => {
    ...
    setSaving(true)
    setValidationNotice([])
    const selectedOwner = selectedOwnerOption?.user || null
    const selectedOwnerName = getOwnerName(selectedOwner)
    const selectedOwnerCode = normalizeOwnerCode(selectedOwner?.ownerCode)
    ...
    const payload = {
      ...
      accountOwnerCode: selectedOwnerCode,
      ...
      ...formData,
      accountOwner: selectedOwnerName,
      contacts: [
        {
