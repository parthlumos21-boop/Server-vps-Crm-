...
const normalizeManagedCompany = (company) => {
  const value = String(company || '').trim().toLowerCase()
  if (value !== 'swati' && value !== 'lumos') {
    throw new AppError('Company must be swati or lumos.', 400)
  }
  return value
}
...
const createAdminManagedUser = async ({ name, email, password, companyId = 1, company, role = 'user', designation, state, city }) => {
  const { nameValue, emailValue, passwordValue } = validateUserPayload({ name, email, password })
  const companyValue = normalizeManagedCompany(company)
