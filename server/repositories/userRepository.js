...
const normalizeCompany = (company) => {
  ...
  if (value === 'swati' || value === 'lumos') return value
  return ''
}

const getOwnerCodeRange = (company) => {
  if (company === 'lumos') {
    return { start: 2001, end: 2999 }
  }

  return { start: 1001, end: 1999 }
}

const getNextOwnerCode = async (company) => {
  const normalizedCompany = normalizeCompany(company)
  const { start, end } = getOwnerCodeRange(normalizedCompany)
  const records = await User
    .find(
      { ownerCode: { $exists: true, $nin: ['', null] } },
      { ownerCode: 1 }
    )
    .lean()
  ...
  const usedCodes = new Set(
    records
      .map((record) => Number(String(record.ownerCode || '').trim()))
      .filter((code) => Number.isInteger(code) && code >= start && code <= end)
  )

  let nextCode = usedCodes.size > 0 ? Math.max(...usedCodes) + 1 : start
  ...
  throw new Error(`No ownerCode is available for ${normalizedCompany || 'swati'} users.`)
}
...
const createUser = async ({ username, name, email, passwordHash, assignedPassword = '', role, companyId = 1, company = '', status = 'pending', isApproved = false, designation = '', state = '', city = '' }) => {
  ...
  const normalizedCompany = normalizeCompany(company)
  const ownerCode = normalizedCompany ? await getNextOwnerCode(normalizedCompany) : ''
  const createPayload = {
    legacyId,
    username,
    name,
  ...
  if (normalizedCompany) {
    createPayload.company = normalizedCompany
    createPayload.ownerCode = ownerCode
  }

  const record = await User.create(createPayload)
